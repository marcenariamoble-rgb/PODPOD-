...
/**
 * Somas de repasse/comissão por vendedor.
 * Se a migração com `valorSaldoRepasse` ainda não foi aplicada na base, usa fallback a partir de bruto − comissão.
 */
async function sumRepasseVendaPorVendedor(vendedorId: string): Promise<{
  ...
  try {
    ...
    return {
      totalComissaoRetida: toNumber(r._sum.valorComissaoRetida),
      totalSaldoRepasse: toNumber(r._sum.valorSaldoRepasse),
    };
  } catch (e) {
    logDevError("sumRepasseVendaPorVendedor", e);
    const r = await prisma.venda.aggregate({
      where: { vendedorId },
      _sum: { valorTotal: true, valorComissaoRetida: true },
    });
    const bruto = toNumber(r._sum.valorTotal);
    const comiss = toNumber(r._sum.valorComissaoRetida);
    return {
      totalComissaoRetida: comiss,
      totalSaldoRepasse: Math.max(0, bruto - comiss),
    };
  }
}
...
async function sumSaldoRepasseGlobal(): Promise<number> {
  try {
    const r = await prisma.venda.aggregate({
      _sum: { valorSaldoRepasse: true },
    });
    return toNumber(r._sum.valorSaldoRepasse);
  } catch (e) {
    logDevError("sumSaldoRepasseGlobal", e);
    const r = await prisma.venda.aggregate({
      _sum: { valorTotal: true, valorComissaoRetida: true },
    });
    const bruto = toNumber(r._sum.valorTotal);
    const comiss = toNumber(r._sum.valorComissaoRetida);
    return Math.max(0, bruto - comiss);
  }
}
...
export async function sumEstoqueCentral(): Promise<number> {
  try {
    ...
    logDevError("sumEstoqueCentral", e);
    return 0;
  }
}
...
export async function sumEstoqueEmPosse(): Promise<number> {
  try {
    ...
    logDevError("sumEstoqueEmPosse", e);
    return 0;
  }
}
